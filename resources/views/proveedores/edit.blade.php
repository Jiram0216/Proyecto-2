...
@section('content')
<div class="padre">
    <h2 class="hijo">Editar Proveedor</h2>
</div>
<div class="row justify-content-center mt-5">
    <div class="col-md-8">
        <form action="{{route('proveedores.update', ['proveedor' => $proveedor->id])}}" enctype="multipart/form-data" method="POST" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input type="text" name="empresa" class="form-control @error('empresa') is-invalid @enderror"
                    id="empresa" placeholder="Nombre de la Empresa" value="{{$proveedor->empresa}}">
                @error('empresa')
            ...
            <div class="form-group">
                ...
                <select name="categoria" class="form-control @error('categoria') is-invalid @enderror">
                    ...
                    <option value="{{ $categoria->id }}" {{$proveedor->categoria_id == $categoria->id ? 'selected' : '' }}>{{$categoria->nombre}}</option>
                    @endforeach
                </select>
                @error('categoria')
            ...
            <div class="form-group">
                ...
                <input type="text" name="contacto" class="form-control @error('contacto') is-invalid @enderror"
                    id="contacto" placeholder="Nombre del contacto" value="{{$proveedor->contacto}}">
                @error('contacto')
                <span class="invalid-feedback d-block" role="alert"><strong>{{$message}}</strong></span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="telefono" class="form-control @error('telefono') is-invalid @enderror"
                    id="telefono" placeholder="Teléfono de contacto" value="{{$proveedor->telefono}}">
                @error('telefono')
                <span class="invalid-feedback d-block" role="alert"><strong>{{$message}}</strong></span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <div class="mt-4">
                    <p>Imagen Actual:</p>
                    <img src="/storage/{{$proveedor->imagen}}" style="width: 300px" alt="Imaagen actual">
                </div>
                @error('imagen')
                <span class="invalid-feedback d-block" role="alert"><strong>{{$message}}</strong></span>
                @enderror
